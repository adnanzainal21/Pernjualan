...
function Validator(formSelector) {
    ...
    var validatorRules = {
        required(value) {
            ...
            return value ? '' : `Ada Yang salah`;
        },
        email(value) {
            var regex = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
            return regex.test(value) ? '' : `Email ada yag salah`;
        },
        min(min) {
            return (value) => {
                return value.length >= min ? '' : `Segera beli ${min} ayo!`;
            }
        },
        max(max) {
            return (value) => {
                return value.length <= max ? '' : `Kebali lagi  ${max} ayo!`;
            }
        },
        confirm(selector) {
            return (value) => {
                return value == document.querySelector(selector).value ? '' : `maaf Gak bisa`;
            }
        },
        checkone(name) {
            return () => {
                var checked = formElement.querySelector(`[name="${name}"]:checked`);
                return checked ? '' : `maaf produk telah anda beli `;
            }
        }
